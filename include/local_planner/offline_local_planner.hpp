 ...
 * @Last Modified time: 2024-06-26 11:14:48
 */
...
class OfflineLocalPlanner : public LocalPlannerInterface {
    public:
         ...
         */
        std::vector<std::vector<std::array<double, 2>>> PathGenerator(
    ...
    public:
        inline std::array<double, 2> GetDestPoint(
                const double& angle, const double& radius,
                const std::array<double, 2>& origin_pos) {
            double radians = angle * M_PI / 180.0;
            double x = radius * cos(radians) + origin_pos[0];
            double y = radius * sin(radians) + origin_pos[1];
